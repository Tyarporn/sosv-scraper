...
'''
TO DO! 
- May not work for Food-X? If so, uncomment the list with only Food-X and comment out the main one
'''
...
for program in programs:
    page_number = 1
    max_pages = 20  # pretty much a bandaid fix, there must be a better way of doing this
    print("Now recording for " + program)
    names = []
    founders = []
    linkedins = []
    websites = []
        ...
        try:
            ...
            with open(program + 'info.csv', 'w', newline='') as file:
                ...
                for company in companies:
                    try:
                        writer.writerow(
                            [company.name, company.founder, company.linkedin, company.website, company.description,
                            company.location])
